...
- Match, visitor, high-score, and profile writes require an authenticated web or Activity session.
...
- Better Auth does not store game progression. Neon Fuse keeps a separate
  `player_profiles` collection keyed by the stable player ID.
- Profile identity fields are refreshed from verified Discord data. Clients may
  update only allowlisted arena preferences; statistics and progression remain server-owned.
